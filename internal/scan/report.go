...
func (p *Pipeline) writeManifest(c counts) error {
	m := struct {
		...
		"passive_unresolved_candidates": CountLines(p.L.Path("01_unresolved.txt")),
		"crawl_candidates":              CountLines(p.L.Path("07_candidates.txt")),
		"crawl_unresolved_candidates":   CountLines(p.L.Path("07_unresolved.txt")),
		"optional_warnings":             CountLines(p.L.Path("optional-warnings.log")),
		"pending_crawl_sources":         CountLines(p.L.Path("07_crawl_pending.txt")),
	}}
...
func (p *Pipeline) writeMarkdown(c counts) error {
	...
	for _, row := range []struct {
		...
		{"Passive candidates retained", CountLines(p.L.Path("01_candidates.txt")), "01_candidates.txt"},
		{"Passive candidates unresolved", CountLines(p.L.Path("01_unresolved.txt")), "01_unresolved.txt"},
		{"Crawl candidates retained", CountLines(p.L.Path("07_candidates.txt")), "07_candidates.txt"},
		{"Crawl candidates unresolved", CountLines(p.L.Path("07_unresolved.txt")), "07_unresolved.txt"},
		{"Optional source warnings", CountLines(p.L.Path("optional-warnings.log")), "optional-warnings.log"},
		{"Pending crawl sources", CountLines(p.L.Path("07_crawl_pending.txt")), "07_crawl_pending.txt"},
	} {
		fmt.Fprintf(&b, "| %s | %d | `%s` |\n", row.label, row.n, row.file)
	}

	b.WriteString("\nOptional source warnings do not invalidate successful enumeration. Unresolved candidates remain separate from resolved output. Budget-truncated crawl work stays pending; continue with `--resume-crawl --crawl-budget 2h` and the same collection exclusion file.\n")
