...
# Features: Smart Port Selection, Visual Recon, Monitor, Stability
# ============================================================
    ...
    for t in tmux pv go amass subfinder puredns httpx naabu katana nuclei waybackurls anew jq gum ffuf gowitness; do
        if ! command -v $t &>/dev/null; then
            ...
            if [ "$t" == "gum" ]; then
                ...
                warn "Missing system tool: $t. Try 'sudo apt install $t'"
            else
...
init_conf() {
    ...
    if [ ! -f "$CONF_FILE" ]; then
        touch "$CONF_FILE"
        if gum confirm "Configure Alerts for LeetSec?"; then
...
setup_cron() {
    ...
    if [ -z "$(crontab -l 2>/dev/null | grep "$SCRIPT_PATH" | grep "$TGT")" ]; then
        if gum confirm "Add to LeetSec Scheduler (Auto-Monitor)?"; then
...
# Safety Check for Resolvers
if [ ! -s "$WL_RES" ]; then die "Failed to download resolvers."; fi
...
if [ -s "$WORK_DIR/master.txt" ]; then
    if [ "$DEEP_SCAN" = true ]; then
        PORTS="-p 1-10000"
        warn "DEEP SCAN enabled. This will take time."
    else
    ...
    httpx -l "$T_LIST" -threads "$HTTPX" -random-agent -retries 2 -timeout 10 -sc -title -tech-detect -ip -cname -server -o "$FINAL_DIR/http.txt" -silent > /dev/null 2>&1
    awk '{print $1}' "$FINAL_DIR/http.txt" | sort $SORT -u > "$WORK_DIR/live.txt"
fi
...
if [ -s "$WORK_DIR/live.txt" ]; then
    if command -v gowitness &>/dev/null; then
        ...
        gowitness file -f "$WORK_DIR/live.txt" -P "$FINAL_DIR/screenshots/" --threads 10 --disable-db --no-http >/dev/null 2>&1
    fi

    # Sniper Fuzzing Logic (Corrected JSON Parse)
    echo -e "${C}    -> Sniper Fuzzing (Critical Files)...${NC}"
    echo -e ".env\n.git/config\n.vscode/sftp.json\n.ds_store\nwp-config.php\nconfig.php\nid_rsa\nshadow\npasswd\n.htaccess\nserver-status\nadmin\nbackup\nphpinfo.php\ndocker-compose.yml" > "$WORK_DIR/sniper.txt"

    run_sniper() {
        url=$1; wl=$2; out=$3
        safe=$(echo "$url" | sed 's|https://||g' | sed 's|http://||g' | sed 's|/|_|g')
        ffuf -u "$url/FUZZ" -w "$wl" -mc 200 -fc 404 -fs 0 -timeout 5 -s -o "$out/snip_${safe}.json" >/dev/null 2>&1
    }
    export -f run_sniper
    
    mkdir -p "$WORK_DIR/sniper_results"
    cat "$WORK_DIR/live.txt" | xargs -P "$PARALLEL" -I {} bash -c "run_sniper '{}' '$WORK_DIR/sniper.txt' '$WORK_DIR/sniper_results'"
    
    # FIX: Safe JQ Parsing
    for f in "$WORK_DIR"/sniper_results/*.json; do
        if [ -s "$f" ]; then
             cat "$f" | jq -r 'select(.results != null) | .results[] | "[SNIPER] " + .url' >> "$RPT_DIR/critical_files.txt" 2>/dev/null
        fi
    done
fi
...
if [ -s "$WORK_DIR/live.txt" ]; then
    ...
    if command -v nuclei &>/dev/null; then
        # 10s Timeout / 2 Retries / Filtered Screen Output
...
CRIT=0; [ -f "$RPT_DIR/critical_files.txt" ] && CRIT=$(wc -l < "$RPT_DIR/critical_files.txt")
...
if [ -t 0 ] && command -v gum >/dev/null; then
    gum style --border double --foreground 212 --align center --width 50 "LEETENUM COMPLETE" "Target: $TARGET" "Subs: $DNS" "Crit Files: $CRIT" "Vulns: $VULN"
else
    echo "Finished. Subs: $DNS | Vulns: $VULN"
fi

notify "LeetEnum: $TARGET | Subs: $DNS | Crit: $CRIT | Vulns: $VULN"
